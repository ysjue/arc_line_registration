...
from Solver.arc_line_registration import homo
from Solver.arc_line_registration import LeastSquare_Solver as Solver
...
trus_bounding_box = np.asarray([100, 100, 20], np.float32) # x,y,z, unit: mm
trus_spots = np.array([[-15,-50, 50]]).T+\
    np.random.rand(3,line_num) * trus_bounding_box[:,None]

trus_radii = np.zeros_like(trus_spots)
trus_x = np.zeros_like(trus_spots)
trus_radii[-1,:] = np.linalg.norm(trus_spots,axis = 0)
trus_x[0, :]    = trus_spots[0, :]  
...
F,error,_,_ = solver1.solve(trus_radii,trus_x, cam_laser_start_spots,cam_N, F0=Freg)#identity_matrix())
x_pred, cam_spots_pred = solver1.output()
print('x_pred1:', x_pred)
print('x gt:', x)
print('translation error:', np.linalg.norm(F[:3,3] - Freg[:3,3]))
angle, direc, _ = rotation_from_matrix(F @ np.linalg.inv(Freg))
print('rotation error (angle, direct, point): ', angle * 180.0/3.1415926, direc)
